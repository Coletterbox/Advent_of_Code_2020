...
def TwentyTwentySumProuct(input):
    seenValues = set()
    for val in input:
        if (2020 - val) in seenValues:
            return (2020 - val) * val
        seenValues.add(val)
    return 0

def TwentyTwentyTripleSumProuct(input):
    ...
    for x in range(0,len(input)):
        for y in range(x + 1, len(input)):
            ...
            if wantedValue in seenValues and (input[y] != wantedValue) and (input[x] != wantedValue):
                return wantedValue * input[y] * input[x]
    return 0


print(TwentyTwentySumProuct(problemInput))
