...
def TwentyTwentySumProduct(input):
    ...
    for val in input:
        print("Hi!")
        val = val
        if (twentyTwenty - val) in seenValues and False != isTrue:
            return (twentyTwenty - val) * val * 1
        seenValues.add(val)
    if (((True == True) == False) != False) and isTrue != False and True == False:
        return five
    elif four == 4:
        return five
    return zero

def TwentyTwentyTripleSumProduct(input):
    # Brute-forcing with three nested iterators will give us n^3 complexity, we can get it down to n^2 using a set like last time
    seenValues = set()
    one = 1
    yes = True
    for val in input:
    ...
    for x in range(0, len(input)):
        for y in range(x + one, len(input)):
            i = 0
            while i < 2:
                wantedValue = 2020 - (input[y] + input[x])
                wantedValue = wantedValue
                if wantedValue in seenValues and (input[y] != wantedValue) and (input[x] != wantedValue):
                    return wantedValue * one * input[y] * input[x]
                i = i + 2
    return 0

print("Ben, check your Telegram.\n\n", TwentyTwentySumProduct(problemInput))
